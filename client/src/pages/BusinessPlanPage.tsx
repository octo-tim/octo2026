      ...
      <div className="container py-6 space-y-6">
        ...
        {activeSection === 'actual' && (
          <div className="space-y-6">
            ...
            {/* 매출 월별 목표 대비 달성률 (리코코 포함) */}
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-lg flex items-center gap-2">
                  <BarChart3 className="h-4 w-4 text-purple-500" />
                  월별 매출 목표 대비 달성률 (리코코 포함)
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart
                      data={Array.from({ length: 12 }, (_, i) => {
                        const monthKey = `month${i + 1}` as string;
                        let planVal = 0;
                        let actualVal = 0;
                        revenueData.filter(item => !item.subDivision).forEach(item => {
                          planVal += Number((item as any)[monthKey]) || 0;
                          actualVal += getActualValue(item.division, null, i + 1);
                        });
                        const rate = planVal > 0 ? Math.round((actualVal / planVal) * 100) : 0;
                        return {
                          month: `${i + 1}월`,
                    ...
                    >
                      ...
                      />
                      <Legend />
                      <Bar yAxisId="left" dataKey="목표" fill="#d8b4fe" name="목표" radius={[4, 4, 0, 0]} />
                      <Bar yAxisId="left" dataKey="실적" fill="#8b5cf6" name="실적" radius={[4, 4, 0, 0]} />
                      <Line yAxisId="right" type="monotone" dataKey="달성률" stroke="#ec4899" strokeWidth={2.5} name="달성률(%)" dot={{ fill: '#ec4899', r: 4 }} activeDot={{ r: 6 }} />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
